...
const ColorHarmonyGenerator: React.FC<Props> = ({ isJP, onAdd, onApply }) => {
  ...
  // drawHandles 内から参照するナチュラル補正用 ref
  const ncEnabledRef   = useRef(true);
  const ncDirectionRef = useRef(0);
  ...
  const st = useRef({
    ...
    S: 1.0,
    L: 0.5,
    hues: buildAnalogousHues(180, 30, 6),
    hub: 180,
    offset: 30,
    apex: 0,
    spread: 30,
  ...
  const drawHandles = useCallback(() => {
    ...
    // Natural correction: center gradient circle
    if (ncEnabledRef.current) {
      const r = R_IN - 6;
      const brightAngle = ((ncDirectionRef.current - 90) * Math.PI) / 180;
      const x1 = CX + r * Math.cos(brightAngle);
      const y1 = CY + r * Math.sin(brightAngle);
      const x2 = CX - r * Math.cos(brightAngle);
      const y2 = CY - r * Math.sin(brightAngle);
  ...
  // ── Mode change ───────────────────────────────────────────────────────────
  const [modeUI, setModeUI] = useState<HarmonyMode>("analogous");
  const [nUI, setNUI] = useState(6);
  ...
  // ── Natural correction ────────────────────────────────────────────────────
  const [naturalCorrection, setNaturalCorrection] = useState(true);
  const [correctionStrength, setCorrectionStrength] = useState(100); // 0-100
  const [correctionMode, setCorrectionMode] = useState<"weak" | "strong">("weak");
  const [correctionDirection, setCorrectionDirection] = useState(0);

  // ── Derived values for render ─────────────────────────────────────────────
  const sat = Math.round(st.current.S * 100);
  const lit = Math.round(st.current.L * 100);
  // 補正: 黄(60°)→明るく / 青(240°)→暗く  cos((h-60)°) で ±1 に正規化
  const CORRECTION_COEFF = correctionMode === "strong" ? 250 : 45;
  const applyNaturalCorrection = (h: number, baseLit: number): number => {
    if (!naturalCorrection) return baseLit;
    const delta =
      Math.cos(((h - correctionDirection - 60) * Math.PI) / 180) * (correctionStrength / 100) * CORRECTION_COEFF;
      ...
      >
        ...
        >
          <input
            ...
            onChange={(e) => {
              ncEnabledRef.current = e.target.checked;
              setNaturalCorrection(e.target.checked);
              draw();
            }}
        ...
        {naturalCorrection && (
          <>
            {(["weak", "strong"] as const).map((m) => (
              <button
                key={m}
                onClick={() => setCorrectionMode(m)}
                style={{
                  fontSize: "0.7rem",
                  padding: "0.1rem 0.4rem",
                  border: `1px solid rgb(100,98,110)`,
                  borderRadius: "3px",
                  background: correctionMode === m ? "rgb(100,98,110)" : "rgb(220,220,226)",
                  color: correctionMode === m ? "white" : "rgb(50,48,60)",
                  cursor: "pointer",
                  flexShrink: 0,
                }}
              >
                {m === "weak" ? (isJP ? "弱" : "Weak") : (isJP ? "強" : "Strong")}
              </button>
            ))}
            <input
              type="range"
        ...
        >
          ...
          <input
            ...
            onChange={(e) => {
              ...
              ncDirectionRef.current = v;
              setCorrectionDirection(v);
              draw();
            }}
            style={{
              flex: 1,
      ...
      >
        ...
        <button
          onClick={() => {
            ...
            ncEnabledRef.current = true;
            ncDirectionRef.current = 0;
            setNaturalCorrection(true);
            setCorrectionStrength(100);
            setCorrectionMode("weak");
            setCorrectionDirection(0);
          }}
          style={{ ...btnBase, background: "rgb(220,220,226)" }}
        >
          {isJP ? "リセット" : "Reset"}
