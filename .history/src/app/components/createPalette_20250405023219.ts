...
function selectColorsByHue(palette: RGB[], numColors: number): RGB[] {
  ...
  while (selectedColors.length < numColors) {
    const targetHue = (colorIndex % 8) * hueSteps;
    ...
    // 色が見つかれば追加
    if (colorInRange && !selectedColors.includes(colorInRange)) {
  ...
  return selectedColors.slice(0, numColors);  // numColorsの数に切り詰めて返す
}
...
// 黒と白に近い色を選ぶ
function selectBlackAndWhite(palette: RGB[]): RGB[] {
  const black: RGB = [0, 0, 0];
  const white: RGB = [255, 255, 255];

  const closestBlack = palette.reduce((prev, curr) => {
    return colorDistance(curr, black) < colorDistance(prev, black) ? curr : prev;
  });
  const closestWhite = palette.reduce((prev, curr) => {
    return colorDistance(curr, white) < colorDistance(prev, white) ? curr : prev;
  });
...
const createPalette = (imageSrc: string, numColors: number = 16): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // 黒と白を選出
      const [black, white] = selectBlackAndWhite(palette);
