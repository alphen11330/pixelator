    ...
    for (let y = 0; y < processedSrc.rows; y++) {
      for (let x = 0; x < processedSrc.cols; x++) {
        ...
        if (colorCounts.has(colorKey)) {
          ...
          cv.cvtColor(new cv.Mat([[[b, g, r]]], cv.CV_8UC3), hsv, cv.COLOR_BGR2HSV);
          if (hsv.data.length < 3) {
            console.warn("HSV conversion failed", hsv);
            continue;
          }
          const hue = hsv.data[0] ?? 0;
          const saturation = hsv.data[1] ?? 0;
