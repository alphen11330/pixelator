const createPalette = (imageSrc: string, numColors: number = 8): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const maxClusters = 1000; // 最大クラスタ数
      const initialK = Math.min(numColors * 4, Math.min(rgbImg.rows * rgbImg.cols, maxClusters));
      ...
      for (let i = 0; i < initialK; i++) {
        ...
        // 彩度と明度の重み付け
        const saturationWeight = 50; // 彩度の重み
        const valueWeight = 0.1; // 明度の重み
      ...
      const selectedColors = colorClusters.slice(0, numColors * 4);
