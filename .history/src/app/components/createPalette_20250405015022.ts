...
function wuQuantization(pixels: RGB[], numColors: number): RGB[] {
  ...
  let clusters: RGB[] = weightedPixels.slice(0, numColors);
  ...
  while (iterationCount < maxIterations && !arraysAreEqual(clusters, prevClusters)) {
    ...
    for (let i = 0; i < weightedPixels.length; i++) {
      ...
      for (let j = 0; j < clusters.length; j++) {
        const dist = colorDistance(weightedPixels[i].color, clusters[j]);
        if (dist < minDist) {
          minDist = dist;
          assignedCluster = j;
        }
    ...
    clusters = clusters.map((_, idx) => {
      ...
      return averageColorWithWeights(clusterPixels);
    });
