...
const PixelArtProcessor: React.FC<Props> = ({
  ...
  colorReduction,
  colorPalette,
  ditherType = "atkinson", // デフォルトはフロイド-スタインバーグ
  ...
  const applyOrderedDithering = (
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const threshold = (bayerMatrix[y % 8][x % 8] - 32) * 2;
      ...
      try {
        ...
        switch (ditherType) {
          case "floydsteinberg":
            ...
            break;
          case "atkinson":
            processedImageData = applyAtkinsonDithering(imageData, paletteRGB);
            break;
          case "ordered":
            processedImageData = applyOrderedDithering(imageData, paletteRGB);
            break;
          case "none":
          default:
            // ディザリングなしの通常の色変換
