const createPalette = (imageSrc: string, numColors: number = 16): string[] => {
  const cv = window.cv;
  const img = new Image();
  img.src = imageSrc;
  ...
  cv.cvtColor(src, rgbaImg, cv.COLOR_RGBA2BGRA);  // RGBA形式に変換

  // 画像を1次元の配列に変換（k-means用）
  const pixelCount = rgbaImg.rows * rgbaImg.cols;
  const samples = new cv.Mat(pixelCount, 3, cv.CV_32F);

  // ピクセルデータをサンプルに変換（アルファチャンネルを無視してRGB部分のみ使用）
  for (let y = 0; y < rgbaImg.rows; y++) {
    for (let x = 0; x < rgbaImg.cols; x++) {
      const pixel = rgbaImg.ucharPtr(y, x);
      const idx = y * rgbaImg.cols + x;

      // RGBA値を取得
      const r = pixel[0];
      const g = pixel[1];
      const b = pixel[2];
      const a = pixel[3] / 255; // アルファ（透過）値を0〜1に正規化

      // 透過部分を無視する
      if (a > 0.1) { // 透過度が低いピクセルは無視
  ...
  const attempts = 5; // 試行回数（増加させて精度を向上）
  ...
  // クラスタごとのピクセル数をカウント
  const clusterCounts = new Array(numColors).fill(0);
  ...
  for (let i = 0; i < numColors; i++) {
    ...
    // 彩度と明度の重み付け
    const saturationWeight = 2.0; // 彩度の重み
    const valueWeight = 0.5; // 明度の重み
