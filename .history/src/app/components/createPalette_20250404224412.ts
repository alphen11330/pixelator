const createPalette = (imageSrc: string, numColors: number = 8): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const initialK = Math.max(numColors *2, Math.min(rgbImg.rows * rgbImg.cols, maxClusters));
      ...
      //       const similarityPenalty = 1 / (1 + colorDist);
      ...
      const selectedColors = colorClusters.slice(0, maxClusters);
