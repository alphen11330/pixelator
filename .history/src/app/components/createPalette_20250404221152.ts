const createPalette = (imageSrc: string, number: number = 8): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const initialK = Math.min(number * 2, rgbImg.rows * rgbImg.cols);
      ...
      for (let i = 0; i < initialK; i++) {
        ...
        // 特徴スコアを計算：出現頻度 + 彩度の重み付け + 明度の重み付け
        const saturationWeight = 20; // 彩度の重み
        const valueWeight = 20; // 明度の重み
      ...
      for (let i = 0; i < number; i++) {
        const closestColor = colorClusters.reduce((prev, curr) => {
          const hueDiff = Math.abs(curr.h - currentHue);
          const adjustedHueDiff = hueDiff > 180 ? 360 - hueDiff : hueDiff;

          return adjustedHueDiff < Math.abs(prev.h - currentHue) ? curr : prev;
        });
