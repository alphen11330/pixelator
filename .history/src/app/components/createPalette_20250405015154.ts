...
// Wu Quantizationによる色空間の量子化（出現頻度も考慮）
function wuQuantization(pixels: RGB[], numColors: number): RGB[] {
  ...
  // 色とその出現頻度でピクセルを作成
  const weightedPixels: { color: RGB, weight: number }[] = [];
  frequencyMap.forEach((freq, key) => {
    const color: RGB = key.split(',').map(Number) as RGB;
    weightedPixels.push({ color, weight: freq });
  });

  // 以下、簡略化したWu Quantizationの擬似コード（k-meansを使用）
  let clusters: { color: RGB, weight: number }[] = weightedPixels.slice(0, numColors); // clustersをオブジェクト型に変更
  ...
  while (iterationCount < maxIterations && !arraysAreEqual(clusters, prevClusters)) {
    ...
    clusters = clusters.map((_, idx) => {
      ...
      return { color: averageColorWithWeights(clusterPixels), weight: 0 }; // ここもオブジェクト型に変更
    });
...
const createPalette = (imageSrc: string, numColors: number = 8): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // 出現頻度を考慮したWu Quantizationで色パレットを生成
