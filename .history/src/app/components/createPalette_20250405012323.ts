...
// 近い色をマージして代表色にまとめる
function mergeSimilarColors(palette: RGB[], maxColors: number, threshold: number = 30): RGB[] {
  const merged: RGB[][] = [];
  ...
  for (const color of palette) {
    ...
    for (const group of merged) {
      const dist = colorDistanceRGB(color, group[0]); // グループの先頭色と比較
      if (dist < threshold) {
        group.push(color); // 近ければ同じグループに追加
        found = true;
        break;
      }
    }
    ...
    if (!found) {
      merged.push([color]); // 新しいグループ作成
    }
  }

  // 各グループの平均色を代表色とする
  const averaged = merged.map(group => averageColor(group));

  // 必要なら色数を制限
  return averaged.slice(0, maxColors);
}
...
const createPalette = (imageSrc: string, numColors: number = 8): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const wuPalette = wuQuantization(pixels, 256); // Wuで256色抽出
      const reducedPalette = mergeSimilarColors(wuPalette, numColors, 30); // 色が近ければマージ
