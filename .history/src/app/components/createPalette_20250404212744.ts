const createPalette = (imageSrc: string, numColors: number = 5): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // 明度の均衡（V値の分布）を加味
      const vValues = colorClusters.map(c => c.v);
      const vAvg = vValues.reduce((sum, v) => sum + v, 0) / vValues.length;
      for (let cluster of colorClusters) {
        const vDiff = Math.abs(cluster.v - vAvg);
        // 明度の均衡を保つためにスコアを調整（平均からの偏差に基づくペナルティ）
        cluster.distinctiveScore -= vDiff * 10;
      }
