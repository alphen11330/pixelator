...
type Props = {
  ...
  setMixBlendMode: React.Dispatch<React.SetStateAction<Property.MixBlendMode>>;
};

const BlendModeDropdown: React.FC<Props> = ({
  mixBlendMode,
  setMixBlendMode,
}) => {
  const blendModes = [
  ...
  const blendModeSelect: React.CSSProperties = {
    width: "50x",
    padding: "5px",
    marginTop: "1rem",
    fontSize: "14px",
  ...
  return (
    <div>
      ...
      <select
        id="blendModeSelect"
        value={mixBlendMode}
        style={blendModeSelect}
        onChange={(e) =>
          setMixBlendMode(e.target.value as Property.MixBlendMode)
        }
      >
        {blendModes.map((mode) => (
