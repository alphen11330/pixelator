...
export default function PixelArtConverter() {
  ...
  useEffect(() => {
    if (imageSrc) {
      const hlsColors = createPallet(imageSrc); // number[][]を返す
      const stringPalette = hlsColors.map((color) => color.join(", ")); // number[]をstringに変換
      setColorPalette(stringPalette); // string[]として状態を更新
    }
  }, [imageSrc]);
  ...
  useEffect(() => {
    ...
    script.src = "https://docs.opencv.org/3.4/opencv.js"; // 最新バージョンのCDNリンク
    script.async = true;
  ...
  useEffect(() => {
    if (imageSrc) {
      ...
      cv["onRuntimeInitialized"] = () => {
        // OpenCV.jsのコードをここで実行
        const mat = cv.matFromImageData(imageSrc);
        alert(typeof mat.reshape); // ここで初期化を確認
      };
    }
  }, [imageSrc]);
