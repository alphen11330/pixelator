...
export default function page() {
  ...
  >("atkinson"); // ディザリング手法の選択
  const [ditherStrength, setDitherStrength] = useState(1); // ディザリング強度

  const [edgeEnhancement, setEdgeEnhancement] = useState(false); // 輪郭線強調の判定
  const [whiteErodeSize, setWhiteErodeSize] = useState(2); // 白画素縮小サイズ
  const [blackDilateSize, setBlackDilateSize] = useState(2); // 黒画素拡大サイズ
  // OpenCV.js をロード
  useEffect(() => {
          ...
          {edgeEnhancement && (
            <div className="ml-7">
              <InputRange
                ...
                max={10}
                step={1}
                value={whiteErodeSize}
                unit={"px"}
                setValue={setWhiteErodeSize}
              />
              <InputRange
                name={"黒画素拡大サイズ"}
                min={1}
                max={10}
                step={1}
                value={blackDilateSize}
                unit={"px"}
                setValue={setBlackDilateSize}
              />
            </div>
          )}
          <CheckBox
      ...
      {smoothImageSrc && imageSrc && (
        <>
          <ImageEditor
            ...
            whiteErodeSize={whiteErodeSize}
            blackDilateSize={blackDilateSize}
          />
        </>
      )}
    </>
