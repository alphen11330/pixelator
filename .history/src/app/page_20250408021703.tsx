...
import { colorReductionProcessor } from "./components/colorReductionProcessor";
...
export default function PixelArtConverter() {
  ...
  // 例：パレットで再描画
  useEffect(() => {
    if (dotsImageSrc && colorReduction && colorPalette.length > 0) {
      colorReductionProcessor(dotsImageSrc, colorPalette, pixelLength).then(
        (newSrc) => {
          setDotsImageSrc(newSrc);
        }
      );
    }
  }, [dotsImageSrc, colorPalette, colorReduction]);
