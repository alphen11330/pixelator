...
const R_OUT = 74, R_IN = 48, MID_R = (R_OUT + R_IN) / 2;
...
const buildAnalogousHues = (hub: number, offset: number, count: number): number[] => {
...
const MODES: {
  label: string; labelJP: string; value: HarmonyMode; fixedCount: boolean; defaultN: number;
}[] = [
  { label: "Tetrad",     labelJP: "テトラード", value: "tetrad",    fixedCount: true,  defaultN: 4 },
  { label: "Split Comp", labelJP: "分裂補色",   value: "splitcomp", fixedCount: true,  defaultN: 3 },
  { label: "Analogous",  labelJP: "類似配色",   value: "analogous", fixedCount: false, defaultN: 4 },
  { label: "Dynamic",    labelJP: "均等配色",   value: "dynamic",   fixedCount: false, defaultN: 6 },
];
...
const ColorHarmonyGenerator: React.FC<Props> = ({ isJP, onAdd, onApply }) => {
  ...
  const dragRef   = useRef<{ type: "hue"; idx: number } | { type: "sl" } | null>(null);
  ...
  useEffect(() => {
    ...
    for (let d = 0; d < 360; d++) {
      ...
      ctx.arc(CX, CY, MID_R, (d + HUE_OFFSET - 1) * Math.PI / 180, (d + HUE_OFFSET + 1) * Math.PI / 180);
      ctx.stroke();
    }
  }, []);
  ...
  const drawSL = useCallback(() => {
    ...
    [0.25, 0.5, 0.75].forEach((t) => {
      ctx.beginPath(); ctx.moveTo(t * pw, 0); ctx.lineTo(t * pw, ph); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0, t * ph); ctx.lineTo(pw, t * ph); ctx.stroke();
    });
  }, []);
  ...
  const drawHandles = useCallback(() => {
    ...
    const sat = Math.round(S * 100), lit = Math.round(L * 100);
    ...
    if (mode === "analogous") {
      hues.forEach((h, i) => {
        ...
        const x = CX + MID_R * Math.cos(r), y = CY + MID_R * Math.sin(r);
    ...
    if (mode === "analogous") {
      ...
      const gx = CX + MID_R * Math.cos(gr), gy = CY + MID_R * Math.sin(gr);
      ctx.setLineDash([3, 5]); ctx.lineWidth = 2; ctx.strokeStyle = "rgba(255,255,255,0.35)";
      ctx.beginPath();
      const r0 = (hues[0] + HUE_OFFSET) * Math.PI / 180;
      ctx.moveTo(gx, gy); ctx.lineTo(CX + MID_R * Math.cos(r0), CY + MID_R * Math.sin(r0));
      const rn = (hues[hues.length - 1] + HUE_OFFSET) * Math.PI / 180;
      ctx.moveTo(gx, gy); ctx.lineTo(CX + MID_R * Math.cos(rn), CY + MID_R * Math.sin(rn));
      ctx.stroke(); ctx.setLineDash([]);
      ctx.save(); ctx.translate(gx, gy); ctx.rotate(Math.PI / 4);
      ctx.beginPath(); ctx.rect(-6, -6, 12, 12);
      ctx.fillStyle = `hsl(${guideAngle},${sat}%,${lit}%)`; ctx.fill();
      ctx.lineWidth = 2; ctx.strokeStyle = "rgba(255,255,255,0.7)"; ctx.stroke();
      ctx.restore();
      ctx.font = "bold 8px monospace"; ctx.fillStyle = "rgba(255,255,255,0.35)";
    ...
    if (mode === "splitcomp") {
      ...
      ctx.setLineDash([2, 6]); ctx.lineWidth = 1; ctx.strokeStyle = "rgba(255,255,255,0.18)";
      ctx.beginPath();
      ctx.moveTo(CX + MID_R * Math.cos(apexRad), CY + MID_R * Math.sin(apexRad));
      ctx.lineTo(CX + MID_R * Math.cos(compRad), CY + MID_R * Math.sin(compRad));
      ctx.stroke(); ctx.setLineDash([]);
      ctx.font = "bold 8px monospace"; ctx.fillStyle = "rgba(255,255,255,0.35)";
      ctx.textAlign = "center";
    ...
    hues.forEach((h, i) => {
      const r = (h + HUE_OFFSET) * Math.PI / 180;
      const hx = CX + MID_R * Math.cos(r), hy = CY + MID_R * Math.sin(r);
      const isApex = mode === "splitcomp" && i === 0;
      ctx.beginPath();
      ctx.arc(hx, hy, isApex ? 9 : 6, 0, Math.PI * 2);
      ctx.fillStyle = `hsl(${h},${sat}%,${lit}%)`; ctx.fill();
      ctx.lineWidth = isApex ? 2.5 : 1.5;
      ctx.strokeStyle = isApex ? "#fff" : "rgba(255,255,255,0.75)"; ctx.stroke();
      if (isApex) {
  ...
  const hitHue = (mx: number, my: number): number => {
    ...
    const angle = (Math.atan2(dy, dx) * 180 / Math.PI - HUE_OFFSET + 360) % 360;
  ...
  const updateHue = (mx: number, my: number, idx: number) => {
    const angle = (Math.atan2(my - CY, mx - CX) * 180 / Math.PI - HUE_OFFSET + 360) % 360;
    const s = st.current;
    if (s.mode === "analogous") {
      if (idx === -2) {
        ...
        if (isEnd) {
          ...
          const t = s.hues.length === 1 ? 1 : Math.abs((idx / (s.hues.length - 1)) * 2 - 1);
          s.offset = Math.max(1, Math.min(120, Math.abs(delta) / (t < 0.01 ? 1 : t)));
          s.hues = buildAnalogousHues(s.hub, s.offset, s.n);
        } else {
  ...
  const onWheelPointerUp = () => { dragRef.current = null; redraw(); };
  ...
  const updateSL = (e: React.PointerEvent) => {
    ...
    st.current.S = Math.max(0.01, Math.min(1.0, (e.clientX - rect.left) / rect.width));
    st.current.L = Math.max(0.05, Math.min(0.95, 1 - (e.clientY - rect.top) / rect.height));
    draw();
  };
  ...
  const onSLPointerUp = () => { dragRef.current = null; redraw(); };
  ...
  const changeMode = (newMode: HarmonyMode, newN: number, resetSL = false) => {
    const s = st.current;
    s.mode = newMode; s.n = newN;
    if (newMode === "tetrad")         s.hues = [0, 90, 180, 270];
    else if (newMode === "splitcomp") { s.apex = 0; s.spread = 30; s.hues = buildSplitCompHues(0, 30); }
    else if (newMode === "analogous") { s.hub = 180; s.offset = 30; s.hues = buildAnalogousHues(180, 30, newN); }
    else                              s.hues = Array.from({ length: newN }, (_, i) => (i * 360 / newN) % 360);
    if (resetSL) { s.S = 1.0; s.L = 0.5; }
    setModeUI(newMode); setNUI(newN); redraw();
  };

  const changeN = (newN: number) => {
    const s = st.current; s.n = newN;
    if (s.mode === "analogous") s.hues = buildAnalogousHues(s.hub, s.offset, newN);
    else s.hues = Array.from({ length: newN }, (_, i) => (i * 360 / newN) % 360);
    setNUI(newN); redraw();
  };
  ...
  const applyNaturalCorrection = (h: number, baseLit: number): number => {
    ...
    const delta = Math.cos((h - 60) * Math.PI / 180) * (correctionStrength / 100) * 45;
  ...
  return (
    <div style={{ userSelect: "none" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
        ...
        >
          {MODES.map((m) => (
            <option key={m.value} value={m.value}>{isJP ? m.labelJP : m.label}</option>
          ))}
        </select>
        {!currentMode.fixedCount && (
          <div style={{ display: "flex", alignItems: "center", gap: "0.3rem", flex: 1 }}>
            <input
              type="range" min={2} max={12} value={nUI}
              onChange={(e) => changeN(+e.target.value)}
              style={{ flex: 1, accentColor: "rgb(60,120,220)", cursor: "pointer" }}
            />
            <span style={{ fontSize: "0.72rem", minWidth: 16, color: "rgb(50,48,60)" }}>{nUI}</span>
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: "0.5rem", height: H + "px" }}>
        ...
        <div style={{ position: "relative", flexShrink: 0, width: W, height: H }}>
          <canvas
            ref={ringRef} width={W} height={H}
            style={{ position: "absolute", top: 0, left: 0, borderRadius: "50%" }}
          />
          <canvas
            ref={topRef} width={W} height={H}
        ...
        <div
          ref={slPadRef}
          style={{
            flex: 1, position: "relative", borderRadius: "8px",
            overflow: "hidden", border: "1px solid rgb(100,98,110)",
        ...
        >
          <canvas
            ref={slCanvasRef}
            style={{ display: "block", position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" }}
          />
          {/* Dot indicator */}
          <div style={{
            position: "absolute",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginTop: "0.5rem" }}>
        <label style={{ display: "flex", alignItems: "center", gap: "0.2rem", fontSize: "0.75rem", cursor: "pointer", userSelect: "none", flexShrink: 0 }}>
          <input
            type="checkbox"
        ...
        {naturalCorrection && (
          <>
            <input
              type="range" min={0} max={100} value={correctionStrength}
              onChange={(e) => setCorrectionStrength(+e.target.value)}
              style={{ flex: 1, accentColor: "rgb(60,120,220)", cursor: "pointer" }}
            />
            <span style={{ fontSize: "0.72rem", minWidth: 24, textAlign: "right", color: "rgb(50,48,60)" }}>
              {correctionStrength}
            </span>
          </>
        )}
      ...
      <div style={{
        ...
        marginTop: "0.5rem", border: "1px solid rgb(100,98,110)",
      }}>
        {paletteHex.map((c, i) => (
      ...
      <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.5rem", justifyContent: "space-between" }}>
        ...
        <button
          onClick={() => changeMode(modeUI, MODES.find((m) => m.value === modeUI)!.defaultN, true)}
          style={{ ...btnBase, background: "rgb(220,220,226)" }}
        >
          {isJP ? "リセット" : "Reset"}
        </button>
