...
function applyLambertBeerTransform(value: number): number {
  ...
  return Math.pow(value / 255, 0.4) * 255;
}

// 逆ランベルト・ベール変換（線形→非線形）
function inverseLambertBeerTransform(value: number): number {
  // 明るい値をさらに多く分布させる
  return Math.pow(value / 255, 1.5) * 255;
}
...
function mergeSimilarColors(colors: RGB[], targetCount: number): RGB[] {
  ...
  colorDistances.sort((a, b) => {
    ...
    const brightnessFactor = Math.pow(a.avgBrightness / 100, 10) - Math.pow(b.avgBrightness / 100, 2);
  ...
  for (const { i, j, avgBrightness } of colorDistances) {
    ...
    // 明るい色のペア（v > 80）はマージしない
    if (avgBrightness > 80 && merged.size < targetCount * 0.3) {
...
function createLambertBeerGradient(width: number = 256, height: number = 50): string {
  ...
  for (let i = 0; i <= 30; i++) {
    const pos = i / 30;
    // より強い指数関数的な色の変化を生成
    const intensity = Math.pow(pos, 2.5); // より急激な変化
