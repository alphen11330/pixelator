  ...
  const applyOrderedDithering = (
    ...
    const applyOrderedDithering = (
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          const i = (y * width + x) * 4;
          if (data[i + 3] < 10) continue; // αチャンネルが小さい場合は無視
    
          const r = data[i];
          const g = data[i + 1];
          const b = data[i + 2];
    
          // RGBごとに少しずつ異なる位置のマトリクスを参照
          const tR = (bayerMatrix[y % 8][x % 8] - 32) * 2 * (strength * 1.05); // 赤は少し強く
          const tG = (bayerMatrix[(y + 1) % 8][(x + 1) % 8] - 32) * 2 * strength;
          const tB = (bayerMatrix[(y + 2) % 8][(x + 2) % 8] - 32) * 2 * (strength * 0.95); // 青は少し弱く
    
          const adjustedR = Math.max(0, Math.min(255, r + tR));
          const adjustedG = Math.max(0, Math.min(255, g + tG));
          const adjustedB = Math.max(0, Math.min(255, b + tB));
