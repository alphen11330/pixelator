 ...
 * @param imgSrc - 画像のDataURL形式または画像へのパス
 * @param colorCount - 抽出する色の数
 * @returns RGB値の文字列配列 (例: ["rgb(255,0,0)", "rgb(0,255,0)"])
 */
export const createPalette = async (imgSrc: string, colorCount: number): Promise<string[]> => {
  try {
    // 画像からピクセルデータを取得
    const pixelData = await getPixelDataFromImage(imgSrc);
