...
const Painter: React.FC<Props> = ({
  ...
  const gridGuid: React.CSSProperties = {
    position: "absolute",
    top: 0,
    left: 0,
    width: "100%",
    height: "100%",
    display: "grid",
    gridTemplateColumns: `repeat(${pixelLength}, 1fr)`,
    gridTemplateRows: `repeat(${pixelLength}, 1fr)`,
    pointerEvents: "none", // グリッドはクリック無効
    zIndex: 10,
  };

  const cell: React.CSSProperties = {
    b  border: "1px solid rgba(0, 0, 0, 0.05)", // 薄く区切る
      ...
      <>
        <div style={paintBoad}>
          ...
          <div style={dotsBox}>
            ...
            <div style={gridGuid}>
              {[...Array(pixelLength * pixelLength)].map((_, i) => (
                <div key={i} style={cell}></div>
              ))}
            </div>
          </div>
