...
const useThrottling = (callback: (...args: any[]) => void, delay: number) => {
...
interface ColorCollectionProcessorProps {
    cv: any;
    src: any;
    isHue: boolean;
    hue: number;
    ...
    brightness: boolean;
    brightnessLevel: number;
    throttleDelay?: number; // スロットリングの遅延時間（ミリ秒）
    onProcessedImage: (processedImage: any) => void; // 処理後の画像を返すコールバック
}

const ColorCollectionProcessorWithThrottling = ({
    cv,
    src,
    isHue,
    hue,
    ...
    brightness,
    brightnessLevel,
    throttleDelay = 300, // デフォルトの遅延時間は300ミリ秒
    onProcessedImage,
}: ColorCollectionProcessorProps) => {
    const throttledProcess = useThrottling(
        (
            currentCv: any,
            currentSrc: any,
            currentIsHue: boolean,
            currentHue: number,
            currentIsLuminance: boolean,
            currentLuminance: number,
            currentIsSaturation: boolean,
            currentSaturation: number,
            currentContrast: boolean,
            currentContrastLevel: number,
            currentBrightness: boolean,
            currentBrightnessLevel: number
        ) => {
    ...
    return null; // このコンポーネントはUIをレンダリングしないためnullを返す
};

export default ColorCollectionProcessorWithThrottling;