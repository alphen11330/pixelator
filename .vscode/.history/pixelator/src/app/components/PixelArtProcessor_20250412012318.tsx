...
const PixelArtProcessor: React.FC<Props> = ({
  ...
  const applyOrderedDithering = (
    ...
    const getPixelChannel = (x: number, y: number, channel: number) => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (data[i + 3] < 10) continue; // αチャンネルが低いものはスキップ

        // ベイヤーしきい値取得（-32 ～ +32 にマッピング）
        const threshold = (bayerMatrix[y % 8][x % 8] - 32) * 2 * strength;

        // R, G, B を横に1ピクセルずらして取得（色収差表現）
        const r = getPixelChannel(x - 1, y, 0);
        const g = getPixelChannel(x, y, 1);
        const b = getPixelChannel(x + 1, y, 2);
