import { useThrottle } from "./useThrottle"; // useThrottle が別ファイルにある場合のインポート

const THROTTLE_DELAY = 100; // スロットリングの間隔（ミリ秒）

export const CollectionProcessor = (
    cv: any,
    src: any,
    isHue: boolean,
    hue: number,
    ...
    // 各パラメータをスロットリング
    const throttledSrc = useThrottle(src, THROTTLE_DELAY);
    const throttledIsHue = useThrottle(isHue, THROTTLE_DELAY);
    const throttledHue = useThrottle(hue, THROTTLE_DELAY);
    const throttledIsLuminance = useThrottle(isLuminance, THROTTLE_DELAY);
    const throttledLuminance = useThrottle(luminance, THROTTLE_DELAY);
    const throttledIsSaturation = useThrottle(isSaturation, THROTTLE_DELAY);
    const throttledSaturation = useThrottle(saturation, THROTTLE_DELAY);
    const throttledContrast = useThrottle(contrast, THROTTLE_DELAY);
    const throttledContrastLevel = useThrottle(contrastLevel, THROTTLE_DELAY);
    const throttledBrightness = useThrottle(brightness, THROTTLE_DELAY);
    const throttledBrightnessLevel = useThrottle(brightnessLevel, THROTTLE_DELAY);
    ...
    if (throttledSrc?.channels() === 4) {
        hasAlpha = true;
        const channels = new cv.MatVector();
        cv.split(throttledSrc, channels);
        ...
        channels.delete();
        merged.delete();
    } else if (throttledSrc) {
        cv.cvtColor(throttledSrc, hls, cv.COLOR_RGB2HLS);
    } else {
        hls.delete();
        if (hasAlpha) alpha.delete();
        return null; // src がまだ初期化されていない場合は処理を行わない
    }
    ...
    for (let y = 0; y < hls.rows; y++) {
        for (let x = 0; x < hls.cols; x++) {
            ...
            if (throttledIsHue) {
                // 現在の色相に回転量を加算
                pixel[0] = (pixel[0] + throttledHue) % 180;  // 色相はOpenCVで0〜179度の範囲
                if (pixel[0] < 0) pixel[0] += 180; // 負の値にならないように調整
            }
            if (throttledIsLuminance) {
                let newLuminance = pixel[1] + throttledLuminance;
                if (newLuminance < 0) newLuminance = 0;
                if (newLuminance > 255) newLuminance = 255;
                pixel[1] = newLuminance;
            }
            if (throttledIsSaturation) {
                let newSaturation = pixel[2] + throttledSaturation;
    ...
    if (throttledContrast || throttledBrightness) {
        const adjustedImage = new cv.Mat();

        // コントラストと明度の調整値を設定
        const contrastFactor = throttledContrast ? throttledContrastLevel : 1.0;
        const brightnessFactor = throttledBrightness ? throttledBrightnessLevel : 0;
