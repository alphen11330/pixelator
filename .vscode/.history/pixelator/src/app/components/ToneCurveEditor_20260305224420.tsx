...
const ToneCurveEditor: React.FC<Props> = ({ points, onChange, isJP }) => {
  ...
  const getPointerPos = (e: React.PointerEvent<SVGSVGElement>): [number, number] => {
  ...
  const handlePointerDown = useCallback((idx: number, e: React.PointerEvent<SVGCircleElement>) => {
    e.preventDefault();
        ...
        {[0.25, 0.5, 0.75].map((frac) => {
          ...
          return (
            <React.Fragment key={frac}>
              <line x1={v} y1={0} x2={v} y2={SVG_SIZE} stroke="rgb(200,200,206)" strokeWidth={0.5} />
              <line x1={0} y1={v} x2={SVG_SIZE} y2={v} stroke="rgb(200,200,206)" strokeWidth={0.5} />
            </React.Fragment>
          );
        })}
        {/* 対角線（デフォルト） */}
        <line x1={0} y1={SVG_SIZE} x2={SVG_SIZE} y2={0} stroke="rgb(160,160,170)" strokeWidth={0.8} strokeDasharray="4,3" />
        {/* カーブ（step=1 で滑らか） */}
        <path d={pathD} fill="none" stroke="rgb(60,120,220)" strokeWidth={1.5} strokeLinejoin="round" strokeLinecap="round" />
        {/* コントロールポイント */}
        {points.map((pt, idx) => {
