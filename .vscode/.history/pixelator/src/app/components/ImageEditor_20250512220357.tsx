...
const ImageEditor: React.FC<Props> = ({
  ...
  // スロットリングを適用する値
  const throttle = 200;
  const throttledHue = useThrottle(hue, 100); // 例: 100ms間隔で更新
  const throttledLuminance = useThrottle(luminance, 100);
  const throttledSaturation = useThrottle(saturation, 100);
  const throttledContrastLevel = useThrottle(contrastLevel, 100);
  const throttledBrightnessLevel = useThrottle(brightnessLevel, 100);
  const throttledWhiteSize = useThrottle(whiteSize, 100);
