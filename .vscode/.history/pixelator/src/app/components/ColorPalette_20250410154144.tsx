...
const readImage = (
  ...
  setImageToPalette: React.Dispatch<React.SetStateAction<string | null>>
) => {
  const MAX_SIZE = 512;
    ...
    if (e.target?.result) {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          setImageToPalette(resizedDataUrl);
        }
      };
    }
  };
...
const CollorPalette: React.FC<Props> = ({
  ...
  const [imageToPalette, setImageToPalette] = useState<string | null>(null);
    ...
    <>
      <div style={buttonContainer}>
        ...
        <input
          ...
          accept="image/*"
          style={{ display: "none" }}
          onChange={(event) => readImage(event, setImageToPalette)}
        />
