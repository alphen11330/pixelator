...
type HLS = [number, number, number]; // 色相、明度、彩度
...
interface ColorBox {
  ...
  max: RGB;
  volume: number;
  lightness: number; // HLSの明度の平均値
}

// ランベルト・ベールの法則に基づく明度マッピング
function lambertBeerLightnessMapping(value: number): number {
  // 指数関数的に減少する明度値を返す
  // 実測値を参考に、指数的に減少する曲線を近似
  const lightnessValues = [100, 97, 93, 88, 82, 75, 68, 60, 50, 35, 0];
  const steps = lightnessValues.length - 1;
  ...
  return lightnessValues[lowerIndex] * (1 - t) + lightnessValues[upperIndex] * t;
}

// RGB値からHLS値に変換
function rgbToHls(r: number, g: number, b: number): HLS {
  r /= 255;
  g /= 255;
  b /= 255;
...
// RGB値から明度（HLSのL）を計算
function getLightness(color: RGB): number {
  const r = color[0] / 255;
  const g = color[1] / 255;
  const b = color[2] / 255;
...
function createBox(colors: RGB[]): ColorBox {
  ...
  colors.forEach(color => {
    ...
    // 明度の計算
    totalLightness += getLightness(color);
  });

  // ボリュームを計算
  const volume = (max[0] - min[0]) * (max[1] - min[1]) * (max[2] - min[2]);
  const lightness = colors.length > 0 ? totalLightness / colors.length : 0;

  return { colors, min, max, volume, lightness };
}
...
// MedianCutアルゴリズムによる色空間の量子化（明度重み付け）
function medianCutQuantization(pixels: RGB[], numColors: number): RGB[] {
  ...
  while (boxes.length < numColors) {
    // ボックスを明度で重み付けしたボリュームでソート
    boxes.sort((a, b) => {
      // 明度とボリュームの両方を考慮した重み付け
      const weightA = a.volume * (1 + a.lightness / 100);
      const weightB = b.volume * (1 + b.lightness / 100);
  ...
  return boxes.map(box => {
    ...
    // 色相と彩度を平均化
    const hlsValues = box.colors.map(c => rgbToHls(c[0], c[1], c[2]));
    
    let sumH = 0;
    let sumL = 0;
    let sumS = 0;
    
    hlsValues.forEach(hls => {
      sumH += hls[0];
      sumL += hls[1];
      sumS += hls[2];
    });
    
    const avgH = sumH / hlsValues.length;
    const avgL = sumL / hlsValues.length;
    const avgS = sumS / hlsValues.length;
    
    // ランベルト・ベールの法則に基づいて明度を調整
    const mappedL = lambertBeerLightnessMapping(avgL / 100) / 100 * 100;
    
    return hlsToRgb(avgH, mappedL, avgS);
  });
}
...
// パレットをHLS（明度）に基づいてソートする
function sortPaletteByHLS(palette: RGB[]): RGB[] {
  // HLSに変換
  const hlsPalette = palette.map(color => {
    const [h, l, s] = rgbToHls(color[0], color[1], color[2]);
    return { rgb: color, hls: [h, l, s] };
  });

  // ランベルト・ベールの法則に基づいて明度をマッピング
  const lightnessDistribution = Array.from({ length: 11 }, (_, i) => i / 10)
    .map(l => lambertBeerLightnessMapping(l));

  // 明度分布に合わせてソート
  return hlsPalette
    .sort((a, b) => {
      // 明度（L）降順
      if (Math.abs(b.hls[1] - a.hls[1]) > 5) return b.hls[1] - a.hls[1];
      // 彩度（S）降順
      if (Math.abs(b.hls[2] - a.hls[2]) > 5) return b.hls[2] - a.hls[2];
      // 色相（H）昇順
...
const createPalette = (imageSrc: string, numColors: number = 16): Promise<string[]> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // HLSの明度に基づいてパレットをソート
      palette = sortPaletteByHLS(palette);
      ...
      palette = palette.map((color, index) => {
        ...
        const newL = lambertBeerLightnessMapping(1 - position);
        return hlsToRgb(h, newL, s);
      });
...
// パレットの明度分布を視覚化
function visualizeLightnessDistribution(palette: RGB[]): void {
  const lightnesses = palette.map(color => {
    const [_, l, __] = rgbToHls(color[0], color[1], color[2]);
    return l;
  });
  
  console.log("パレットの明度分布:", lightnesses);
  
  // 理想的なランベルト・ベール分布との比較
  const idealDistribution = Array.from({ length: palette.length }, (_, i) => {
    const position = i / (palette.length - 1);
    return lambertBeerLightnessMapping(1 - position);
  });
  
  console.log("理想的な明度分布:", idealDistribution);
}

export default createPalette;
export { medianCutQuantization, lambertBeerLightnessMapping, visualizeLightnessDistribution };